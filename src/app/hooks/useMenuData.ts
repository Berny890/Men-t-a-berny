...
export const useMenuData = () => {
  ...
  const loadAll = useCallback(async () => {
    setLoading(true);
    const [{ data: cats }, { data: dishRows }, { data: ingRows }] = await Promise.all([
  ...
  const addDish = async (categoryId: string, name: string, description: string, price: number) => {
    const id = Date.now().toString();
    const newDish: Dish = { id, categoryId, name, description, price, ingredients: [], margin: 30, deliveryCost: 0 };
    setDishes((prev) => [...prev, newDish]);
    await supabase.from('dishes').insert({
      id, category_id: categoryId, name, description, price, margin: 30, delivery_cost: 0,
    });
    return newDish;
  };
  ...
  const updateIngredient = async (dishId: string, ingredientId: string, updates: Partial<Ingredient>) => {
    ...
    if (Object.keys(db).length > 0) {
      await supabase.from('ingredients').update(db).eq('id', ingredientId);
    }
  };
  ...
  const deleteIngredient = async (dishId: string, ingredientId: string) => {
    setDishes((prev) =>
      prev.map((d) =>
        d.id === dishId
          ? { ...d, ingredients: d.ingredients.filter((i) => i.id !== ingredientId) }
          : d
      )
    );
  ...
  const resetToDefault = async () => {
    if (!confirm('¿Borrar todo y cargar los datos de ejemplo? Esta acción no se puede deshacer.')) return;
    ...
    for (const dish of def.dishes) {
      await supabase.from('dishes').insert({
        id: dish.id, category_id: dish.categoryId, name: dish.name,
        description: dish.description, price: dish.price, margin: dish.margin, delivery_cost: dish.deliveryCost,
      });
      for (const ing of dish.ingredients) {
        await supabase.from('ingredients').insert({
          id: ing.id, dish_id: dish.id, name: ing.name,
          quantity: ing.quantity, unit_price: ing.unitPrice, subtotal: ing.subtotal,
        });
      }
    }
    await loadAll();
